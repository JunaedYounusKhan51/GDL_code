...
class DataLoader():
    ...
    def load_data(self, domain, batch_size=1, is_testing=False):
        ...
        path = glob('/content/GDL_code/data/%s/%s/*' % (self.dataset_name, data_type))
    ...
    def load_batch(self, batch_size=1, is_testing=False):
        ...
        path_A = glob('/content/GDL_code/data/%s/%sA/*' % (self.dataset_name, data_type))
        path_B = glob('/content/GDL_code/data/%s/%sB/*' % (self.dataset_name, data_type))
...
def load_fashion_mnist(input_rows, input_cols, path='/content/GDL_code/data/fashion/fashion-mnist_train.csv'):
    #read the csv data
    df = pd.read_csv(path)
...
def load_safari(folder):
    ...
    mypath = os.path.join("/content/GDL_code/data", folder)
    txt_name_list = []
...
def load_celeb(data_name, image_size, batch_size):
    data_folder = os.path.join("/content/GDL_code/data", data_name)
...
def load_music(data_name, filename, n_bars, n_steps_per_bar):
    file = os.path.join("/content/GDL_code/data", data_name, filename)
...
def preprocess_image(data_name, file, img_nrows, img_ncols):
    ...
    image_path = os.path.join('/content/GDL_code/data', data_name, file)
